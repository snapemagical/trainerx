...
function AddStudents(props) {
  ...
  useEffect(()=>{
    const {params} = match
    const student = context?.license?.student
    setFromType(params?.id)
    if(params?.id !== 'add'){
      let data = student?.filter(d=>d.id === +params?.id)[0]
      setState({
        ...state,
  ...
  const onSubmit = () => {
    ...
    Fetch(`accounts/user-invite/${formType === 'add'?'':formType+'/'}`,{
      ...
      if(d.status){
        swal(`${formType === 'add' ? 'New Student Add':'Student Update'} Successfully`, "", "success", {
          button: "ok",
        }).then(d=>{
          ...
          <Paper elevation={3} style={{ marginTop: '20px' }}>
            ...
            >
              <div className="customHeadingMar">
                <h2>{formType === 'add' ?'Add':'Edit'} Students</h2>
                <Divider />
              </div>
              {!loader?<NewProfileAdd formType={formType} formValid={formValid} state={state} onChange={onChange} />:null}
            </div>
          </Paper>
          <Stack
            spacing={2}
...
export default withContext(AddStudents)
