...
function Students(props) {
  ...
  useEffect(() => {
    Fetch('accounts/user-invite/student-list/').then(d => {
      if (d?.status) {
        props.context.addlicense(d.data,'student')
        setData(d.data)
      }
    })
  }, [])
                  ...
                  {data?.length ? data.map((row,index) => (
                    <StyledTableRow key={row.id}>
                      ...
                      <StyledTableCell align="center">
                        ...
                        <Button
                          variant="contained"
                          onClick={()=>props.history.push(`/students/list/${row.id}`)}
                          type="button"
                          style={{
                              marginLeft: "10px",
                              padding: '0',
                        ...
                        <Button
                          variant="contained"
                          onClick={()=>props.history.push(`/user-profile?profileId=${row.id}`)}
                          type="button"
                          style={{
                              marginLeft: "10px",
                              padding: '0',
